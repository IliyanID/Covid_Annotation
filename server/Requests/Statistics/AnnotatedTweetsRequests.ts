...
export const annotatedTweetsRequests = (app:Express) =>{
    ...
    app.post('/api/statistics/annotatedTweets',jsonParser,(req:Request,res:Response)=>{
        log(req)
        let body = req.body as {filter:ICondition[],limit:number}
        database.get_annotated_tweets(body.filter,body.limit).then(result => res.json(result));
    })
}